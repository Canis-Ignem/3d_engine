...
void Light::placeScene() {
	...
	if(m_type == directional){
		...
		//si es direccional dejamos las coordenadas de la camara normalizadas
		m_positionEye = modelView.transformVector(m_position);
		m_positionEye.normalize();

	}
	else if(m_type == spotlight){
		// si es spotlight normalizamos la direccion del spot pero no la posicion
		...
		// si es posicional simplemente se hace el cambio de coordenadas
		m_positionEye = modelView.transformPoint(m_position);

	}else{
		printf("no es ningun tipo de luz");
	}
