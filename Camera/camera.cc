...
void PerspectiveCamera::updateProjection() {
	...
	// trasparencia 12
	// matriz de proyeccion es una variable ya, *m_projTrfm hay que adaptarla
	// esa parte esta en math/trfm buscar projection setFrustrum
	//m_projTrfm.setFrustrum
...
void Camera::updateFrame () {
	/* =================== PUT YOUR CODE HERE ====================== */
	// no perder el valor de m_up usar un auxiliar y normalizar ese
	// hay un comando para normalizar
	m_D = m_E - m_At;
	m_D.normalize();
	m_U = m_Up;
	m_U.normalize();
	m_R = crossVectors(m_U,m_D);
	m_R.normalize();
...
//          values)
//el mfPlanes tiene su tamañao maximo en el .h mirar parametros 
//MAX_CLIP_PLANES
...
int Camera::checkFrustum(const BBox *theBBox, unsigned int *planesBitM) {
	...
	for(int i = 0; i < MAX_CLIP_PLANES; i++){
		...
		intersect = BBoxPlaneIntersect(theBBox,m_fPlanes[i]);
