...
int BSpherePlaneIntersect(const BSphere *bs, Plane *pl) {
	//Se compara la distancia del centro con el radio de la esfera

	printf("BSpherePlaneIntersect from intersect");
	float d = pl->signedDistance(bs->getPosition());
	if(abs(d)<=bs->getRadius()){
		return IINTERSECT;
	}else if(d<0){
		//Si la distancia al plano es negativa esta en el lado negativo de este y viceversa
		return -IREJECT;
	}else return +IREJECT;
...
int  BBoxBBoxIntersect(const BBox *bba, const BBox *bbb ) {
	...
	//QUIZA SE PUEDA HACER CON OR ||
	printf("BBoxBBoxIntersect from intersect");
	if(bba->m_min[0] > bbb->m_max[0] ||
	   bba->m_min[1] > bbb->m_max[1] ||
	   bba->m_min[2] > bbb->m_max[2]){		
			//Se rechaza si algun de los mins es mayor que los max de la otra
			return IREJECT;	
	}
	if (bbb->m_min[0] > bba->m_max[0] ||
		bbb->m_min[1] > bba->m_max[1] ||
		bbb->m_min[2] > bba->m_max[2]){
...
//   -IREJECT inside
//    IINTERSECT intersect
//COREGIIIIIIIIIIIIIIIIIIIIIIIIIIR
int  BBoxPlaneIntersect (const BBox *theBBox, Plane *thePlane) {
	...
	for(i = 0; i < 3 ; i++) {
		if (thePlane->m_n[i] >= 0.0f) {
			//Dependiendo de la normal del plano se asocian los valores max y min the la BB
			Vmin[i] = theBBox->m_min[i];
			Vmax[i] = theBBox->m_max[i];
		} else {
			//Aqui van invertidos noral < 0
			Vmin[i] = theBBox->m_max[i];
			Vmax[i] = theBBox->m_min[i];
		}
	}
	min = thePlane->whichSide(Vmin);

	if (min > 0){
		//Si el minimo es mayor que 0 no corta y lado positivo
		return IREJECT;
	}
	max = thePlane->whichSide(Vmax);
	if (max < 0){
		//Si el maximo es menor que 0 no corta y lado negativo
		return -IREJECT;
	}
	return IINTERSECT; 
}
