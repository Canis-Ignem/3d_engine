...
//	Las transformaciones son el resultado de esto

/*     | c1.x*s  c2.x*s  c3.x*s tr.x | * | P.x |*/
...
Vector3 Trfm3D::transformPoint(const Vector3 & P) const {
	Vector3 res = Vector3();
	printf("transformPoint from trfm3D");
	//le aplica una transformacion a un punto (x,y,z,1)
...
Vector3 Trfm3D::transformVector(const Vector3 & V) const {
	Vector3 res;
	printf("transformVector from trfm3D");
	//le aplica una transformacion a un vector (x,y,z,0)
...
void Trfm3D::setRotAxis(const Vector3 & V, const Vector3 & P, float angle ) {
	...
	printf("setRotAxis from trfm3D");
	
	// como OpenGl multiplica por la derecha primero se añade la ultima transformacion
	this->setTrans(P);
	Trfm3D t1;
	//rotamos en el centro una vez nos hemos hacercado al punto
	t1.setRotVec(V,angle);
	this->add(t1);
	...
	P1[1] = -P[1];
	P1[2] = -P[2];
	//Primero transicion por eso va al final nos ponemos en el punto
	t1.setTrans(P1);
	this->add(t1);
